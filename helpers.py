...
def hyperbola_fit_sideways(c: "Candidate") -> Tuple[float, Optional[np.ndarray]]:
    ...
    x0_guess = np.min(x)
    y0_guess = c.center_y  # Use the blob's center Y coordinate

    def sideways_hyperbola(y, x0, y0, a, b):
        return x0 + a * (np.sqrt(1.0 + ((y - y0) / (b + 0.1)) ** 2) - 1.0)

    try:
        # Tighten bounds on y0 to keep apex near the blob center
        y_margin = (np.max(y) - np.min(y)) * 0.2  # 20% margin
        bounds = (
            [np.min(x) - 50, c.center_y - y_margin, 0.1, 0.1],  # lower bounds
            [np.min(x) + 50, c.center_y + y_margin, 2000.0, 500.0],  # upper bounds
        )
